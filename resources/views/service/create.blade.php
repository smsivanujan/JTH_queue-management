...
@section('title', 'Add Service - SmartQueue')
    ...
    <div class="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                <div>
                    <h1 class="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">Add Service</h1>
                    <p class="text-xs sm:text-sm text-gray-600 mt-1">Create a new service for your organization</p>
                </div>
        ...
        <div class="bg-white rounded-xl sm:rounded-2xl shadow-lg border-2 border-gray-200 p-6 sm:p-8 fade-in">
            <form action="{{ route('app.services.store') }}" method="POST">
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    <p class="mt-1 text-xs sm:text-sm text-gray-500">Choose a descriptive name for this service that identifies the type of queue.</p>
                    @error('name')
                ...
                <div class="mb-4 sm:mb-6">
                    ...
                    <input type="text" 
                           ...
                           minlength="4"
                           class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-gray-900 text-base touch-manipulation"
                           placeholder="Leave empty to require password later">
                    <p class="mt-1 text-xs sm:text-sm text-gray-500">Minimum 4 characters. This password protects access to the service queue. Leave empty if you don't want password protection.</p>
                    @error('password')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
